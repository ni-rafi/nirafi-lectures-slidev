    ...
    <footer className="mt-auto border-t bg-background py-4 px-6 text-center text-xs text-muted-foreground">
      <div className="flex flex-col items-center justify-between gap-2 sm:flex-row max-w-7xl mx-auto">
        <div className="flex flex-col items-center sm:items-start">
          <span className="font-semibold text-foreground">CEE Lectures Portal</span>
          <span>Department of Civil and Environmental Engineering</span>
          <span>Shahjalal University of Science & Technology</span>
        </div>
        <div className="flex flex-col items-center sm:items-end">
          <span>&copy; {currentYear} CEE-SUST. All rights reserved.</span>
          <span>Developed for Civil Engineering course slide presentation.</span>
        </div>
      </div>
    </footer>
  );
